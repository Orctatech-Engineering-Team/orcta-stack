...
Everything opt-in beyond the basics — each section only activates once you add
...
Authentication is always on — no env var required. Powered by
[better-auth](https://better-auth.com) with a Drizzle adapter.

### What's included

- Email + password sign-up / sign-in
- Session management (7-day expiry, rolling 1-day refresh)
- A `role` field on every user (default `"buyer"`)
- Auth routes mounted at `/api/auth/**`
- OpenAPI docs auto-generated for all auth endpoints

### Session in a handler

The `authMiddleware` (applied to all routes in the `routes` array) populates
`c.get("session")`:
...
export const myHandler: AppRouteHandler<MyRoute> = async (c) => {
  ...
  const role = session.user.role; // "buyer" | "seller" | "admin"
  // ...
};
```
...
Add fields in `apps/backend/src/lib/auth.ts` under `user.additionalFields`, then
add the matching column to `packages/db/src/schema/users.ts` and run a
migration.
...
**Requires env vars:** at least one of `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`
or `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET`

Providers are activated only when both their vars are present — leaving vars
blank will not break the app, the button just will not appear. No DB migration
needed (the existing `accounts` table already handles OAuth tokens).
...
Upload files directly to S3 or Cloudflare R2 using presigned URLs. Files never
touch the server.

**Requires env vars:** `S3_ENDPOINT`, `S3_BUCKET`, `S3_REGION`,
...
> **Production note** — `wsManager` is in-process. If you run multiple server
> instances, connections are not shared across them. Add a Redis pub/sub layer
> if you need cross-instance broadcasting.
...
Each worker runs with **concurrency 5** and handles graceful shutdown on
`SIGTERM`/`SIGINT`.
...
In production, run this as a separate process or container alongside the HTTP
server.
...
> **Production note** — the store is in-memory and resets on restart. It is not
> shared across multiple server instances. For multi-instance deployments,
> replace the store with a Redis-backed implementation (e.g.
> `rate-limit-redis`).
...
export function invoiceEmail(
  { amount, dueDate }: { amount: number; dueDate: string },
): EmailTemplate {
  return {
    subject: `Invoice for $${amount}`,
    html: baseTemplate(
      "Invoice",
      `<p>Amount due: $${amount}</p><p>Due by: ${dueDate}</p>`,
    ),
    text: `Invoice\n\nAmount due: $${amount}\nDue by: ${dueDate}`,
  };
}
...
`baseTemplate(title, body)` handles the outer HTML shell and sign-off. All
templates export `{ subject, html, text }`.

---

## Redis

A shared `ioredis` client is available for caching, pub/sub, or anything else
that needs Redis.
...
```typescript
import { paginatedSuccessSchema, paginationSchema } from "@repo/shared";
...
**Requires env var:** `REDIS_URL` — degrades gracefully to a direct function
call when Redis is absent.

### API

```typescript
import { cacheKey, invalidateCache, withCache } from "@/lib/cache";
```
